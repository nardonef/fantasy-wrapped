...
export async function fetchYahooPayloads(
  api: YahooApi,
  leagueKey: string,
): Promise<YahooLeaguePayloads> {
  ...
  const teamKeys = extractStandingsTeams(leagueData?.standings).map(
    (t) => (t as { team_key: string }).team_key,
  );
  ...
  const [scoreboards, rosters, transactions, draftResults] = await Promise.all([
    ...
    Promise.all(
      teamKeys.flatMap((teamKey) =>
        weeks.map((w) => api.getRoster(teamKey, w).then((r) => [teamKey, w, r] as const)),
      ),
    ),
