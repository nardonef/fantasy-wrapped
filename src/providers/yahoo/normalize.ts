...
export function normalizeYahooLeague(payloads: YahooLeaguePayloads): NormalizedLeagueBundle {
  ...
  for (const week of weekNumbers) {
    ...
    for (const matchup of scoreboard.matchups as YahooScoreboardMatchup[]) {
      // teamA must be the lower providerRosterId (see NormalizedMatchup's doc
      // comment and the unique index in db/schema.ts) — Yahoo's own team order
      // in the response isn't guaranteed to satisfy that.
